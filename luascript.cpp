...
void LuaInterface::registerFunctions()
{
	...
	//getCreatureShield(cid)
	lua_register(m_luaState, "getCreatureShield", LuaInterface::luaGetCreatureShield);

	//getCreatureMaxShield(cid[, ignoreModifiers = false])
	lua_register(m_luaState, "getCreatureMaxShield", LuaInterface::luaGetCreatureMaxShield);

	//getCreatureBarrier(cid)
	lua_register(m_luaState, "getCreatureBarrier", LuaInterface::luaGetCreatureBarrier);

	//getCreatureMaxBarrier(cid[, ignoreModifiers = false])
	lua_register(m_luaState, "getCreatureMaxBarrier", LuaInterface::luaGetCreatureMaxBarrier);
	...
	//doCreatureAddShield(cid, shield)
	lua_register(m_luaState, "doCreatureAddShield", LuaInterface::luaDoCreatureAddShield);

	//doCreatureAddBarrier(cid, barrier)
	lua_register(m_luaState, "doCreatureAddBarrier", LuaInterface::luaDoCreatureAddBarrier);
	...
	//setCreatureMaxShield(cid, shield)
	lua_register(m_luaState, "setCreatureMaxShield", LuaInterface::luaSetCreatureMaxShield);
	
	//setCreatureMaxBarrier(cid, barrier)
	lua_register(m_luaState, "setCreatureMaxBarrier", LuaInterface::luaSetCreatureMaxBarrier);
	...
	lua_register(m_luaState, "getHouseTiles", LuaInterface::luaGetHouseTiles);	
...
int32_t LuaInterface::luaDoCreatureAddShield(lua_State* L)
{
	//doCreatureAddShield(uid, shield)

	int32_t shieldChange = popNumber(L);
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
...
int32_t LuaInterface::luaDoCreatureAddBarrier(lua_State* L)
{
	//doCreatureAddBarrier(uid, barrier)

	int32_t barrierChange = popNumber(L);
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
...
int32_t LuaInterface::luaGetHouseTiles(lua_State* L)
{
	//getHouseTiles(houseId)
	if(House* house = Houses::getInstance()->getHouse(popNumber(L)))
	{
...
int32_t LuaInterface::luaGetMonsterInfo(lua_State* L)
{
	...
	setField(L, "shield", mType->shield);
	setField(L, "shieldMax", mType->shieldMax);
...
int32_t LuaInterface::luaGetCreaturePartyShield(lua_State* L)
{
	...
	{
		if(!tid)
			lua_pushnumber(L, creature->getPShield());
		else if(Creature* target = env->getCreatureByUID(tid))
...
int32_t LuaInterface::luaDoCreatureSetPartyShield(lua_State* L)
{
	...
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		creature->setPShield(shield);
		g_game.updateCreaturePShield(creature);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaInterface::luaDoPlayerAddDialog(lua_State* L)
{
	//doPlayerAddDialog(dialog, dialogId, cid, callback)
	ScriptEnviroment* env = getEnv();
	LuaInterface* interface = env->getInterface();
	if(!interface)
	{
...
int32_t LuaInterface::luaGetCreatureShield(lua_State* L)
{
	//getCreatureShield(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
...
int32_t LuaInterface::luaGetCreatureMaxShield(lua_State* L)
{
	//getCreatureMaxShield(cid[, ignoreModifiers = false])
	bool ignoreModifiers = false;
	if(lua_gettop(L) > 1)
	...
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
		lua_pushnumber(L, creature->getPlayer() && ignoreModifiers ? creature->shieldMax : creature->getMaxShield());
	else
	{
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaInterface::luaGetCreatureBarrier(lua_State* L)
{
	//getCreatureBarrier(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
		lua_pushnumber(L, creature->getBarrier());
	else
	{
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaInterface::luaGetCreatureMaxBarrier(lua_State* L)
{
	//getCreatureMaxBarrier(cid[, ignoreModifiers = false])
	bool ignoreModifiers = false;
	if(lua_gettop(L) > 1)
	...
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
		lua_pushnumber(L, creature->getPlayer() && ignoreModifiers ? creature->barrierMax : creature->getMaxBarrier());
	else
	{
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaInterface::luaSetCreatureMaxShield(lua_State* L)
{
	//setCreatureMaxShield(uid, shield)
	uint32_t maxShield = (uint32_t)popNumber(L);

	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		creature->changeMaxShield(maxShield);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaInterface::luaSetCreatureMaxBarrier(lua_State* L)
{
	//setCreatureMaxBarrier(uid, barrier)
	uint32_t maxBarrier = (uint32_t)popNumber(L);

	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		creature->changeMaxBarrier(maxBarrier);
		lua_pushboolean(L, true);
	}
	else
	{
