...
void Player::setVocation(uint32_t id)
{
	...
	{
		int hpgain = vocation->getGainAmount(GAIN_HEALTH);
		std::string valuea;
		getStorage("healthregen", valuea);
		hpgain = hpgain + atoi(valuea.c_str());
		int mpgain = vocation->getGainAmount(GAIN_MANA);
		std::string valueb;
		getStorage("manaregen", valueb);
		mpgain = mpgain + atoi(valueb.c_str());
		int shgain = vocation->getGainAmount(GAIN_SHIELD);
		std::string valuec;
		getStorage("shieldregen", valuec);
		shgain = shgain + atoi(valuec.c_str());
		condition->setParam(CONDITIONPARAM_HEALTHGAIN, hpgain);
		condition->setParam(CONDITIONPARAM_HEALTHTICKS, (vocation->getGainTicks(GAIN_HEALTH) * 1000));
		condition->setParam(CONDITIONPARAM_MANAGAIN, mpgain);
		condition->setParam(CONDITIONPARAM_MANATICKS, (vocation->getGainTicks(GAIN_MANA) * 1000));
		condition->setParam(CONDITIONPARAM_SHIELDGAIN, shgain);
		condition->setParam(CONDITIONPARAM_SHIELDTICKS, (vocation->getGainTicks(GAIN_SHIELD) * 1000));
	}
}
...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	if(lookDistance == -1)
	{
		...
		else if(vocationId != VOCATION_NONE&& (g_config.getBool(ConfigManager::SHOW_PLAYER_VOCATION)))
			s << " You are " << vocation->getDescription();
		else if(g_config.getBool(ConfigManager::SHOW_PLAYER_VOCATION))
	...
	else
	{
		...
		if(hasFlag(PlayerFlag_ShowGroupNameInsteadOfVocation))
			...
			s << ". " << (sex % 2 ? "He" : "She") << " is " << vocation->getDescription();
		else if(g_config.getBool(ConfigManager::SHOW_PLAYER_VOCATION))
			s << ". " << (sex % 2 ? "He" : "She") << " has no vocation";
	}

	std::string tmp;
	if(marriage && IOLoginData::getInstance()->getNameByGuid(marriage, tmp)&& (g_config.getBool(ConfigManager::SHOW_PLAYER_MARRIAGE)))
	{
		s << ", ";
	...
	s << ".";
	if(guildId && (g_config.getBool(ConfigManager::SHOW_PLAYER_GUILD)))
	{
		if(lookDistance == -1)
			s << " You are ";
		else
...
int32_t Player::getArmor() const
{
	...
	if(vocation->getMultiplier(MULTIPLIER_ARMOR) != 1.0)
		armor = (armor * vocation->getMultiplier(MULTIPLIER_ARMOR));

	std::string value;
	if(getStorage("armor", value))
		armor = (int32_t)std::ceil(atoi(value.c_str()));
		
	std::string value2;
	if(getStorage("armorPercent", value2))
		armor *= (int32_t)std::ceil(atoi(value2.c_str())/100);
...
int32_t Player::getDefense() const
{
	...
	double defenseFactor = getDefenseFactor();
	...
	if(!g_config.getBool(ConfigManager::USE_SKILLS))
		defenseValue = ((int32_t)std::ceil(defenseValue * defenseFactor));
	else
		defenseValue = ((int32_t)std::ceil(((float)(defenseSkill * (defenseValue * 0.015)) + (defenseValue * 0.1)) * defenseFactor));
	
	std::string value;
	if(getStorage("defense", value))
		defenseValue = (int32_t)std::ceil((atoi(value.c_str())) * defenseFactor);	
	
	std::string value2;
	if(getStorage("defensePercent", value2))
		defenseValue *= (int32_t)std::ceil((atoi(value2.c_str())/100));	
...
double Player::getAttackFactor() const
{
	switch(fightMode)
	{
		case FIGHTMODE_BALANCED:
			return g_config.getDouble(ConfigManager::BALANCED_DAMAGE);

		case FIGHTMODE_DEFENSE:
			return g_config.getDouble(ConfigManager::DEFENSE_DAMAGE);
	...
	return g_config.getDouble(ConfigManager::OFFENSE_DAMAGE);
}
...
double Player::getDefenseFactor() const
{
	...
	{
		case FIGHTMODE_BALANCED:
			return g_config.getDouble(ConfigManager::BALANCED_DEFENSE);

		case FIGHTMODE_DEFENSE:
			return g_config.getDouble(ConfigManager::DEFENSE_DEFENSE);
	...
	return g_config.getDouble(ConfigManager::OFFENSE_DEFENSE);
}
...
int32_t Player::getPlayerInfo(playerinfo_t playerinfo) const
{
	switch(playerinfo)
	{
		...
		case PLAYERINFO_SHIELD:
			return shield;
		case PLAYERINFO_MAXSHIELD:
			return std::max((int32_t)1, ((int32_t)shieldMax + varStats[STAT_MAXSHIELD]));
		case PLAYERINFO_BARRIER:
			return barrier;
		case PLAYERINFO_MAXBARRIER:
			return std::max((int32_t)1, ((int32_t)barrierMax + varStats[STAT_MAXBARRIER]));
	...
	switch(stat)
	{
		...
		case STAT_MAXSHIELD:
		{
			if(getShield() > getMaxShield())
				Creature::changeShield(getMaxShield() - getShield());

			break;
		}

		case STAT_MAXBARRIER:
		{
			if(getBarrier() > getMaxBarrier())
				Creature::changeBarrier(getMaxBarrier() - getBarrier());
...
int32_t Player::getDefaultStats(stats_t stat)
{
	switch(stat)
	{
		...
		case STAT_MAXSHIELD:
			return getMaxShield() - getVarStats(STAT_MAXSHIELD);
		case STAT_MAXBARRIER:
			return getMaxBarrier() - getVarStats(STAT_MAXBARRIER);
		case STAT_SOUL:
...
void Player::addExperience(uint64_t exp)
{
	...
	while(experience >= nextLevelExp)
	{
		...
		shieldMax += voc->getGain(GAIN_SHIELD);
		shield += voc->getGain(GAIN_SHIELD);
...
void Player::removeExperience(uint64_t exp, bool updateStats/* = true*/)
{
	...
	while(level > 1 && experience < Player::getExpForLevel(level))
	{
		...
		shieldMax = std::max((int32_t)0, (shieldMax - (int32_t)voc->getGain(GAIN_SHIELD)));
...
void Player::onBlockHit(BlockType_t)
{
	...
	{
		--shieldBlockCount;
		if(hasShield() && g_config.getBool(ConfigManager::USE_SKILLS))
	...
	bool checkDefense/* = false*/, bool checkArmor/* = false*/, bool reflect/* = true*/, bool field/* = false*/, bool element/* = false*/)
{
	...
	{
		std::string value;
		getStorage("physicalresist", value);
		blocked += (int32_t)std::ceil(damage * (atoi(value.c_str())/100));
	}
	else
	{
		std::string value;
		getStorage("magicresist", value);
		blocked += (int32_t)std::ceil(damage * (atoi(value.c_str())/100));
	}
...
bool Player::onDeath()
{
	...
	if(skillLoss)
	{
		...
		else if(!inventory[SLOT_BACKPACK] && g_config.getNumber(ConfigManager::DEATH_CONTAINER) < 0) // FIXME: you should receive the bag after you login back...
...
ReturnValue Player::__queryAdd(int32_t index, const Thing* thing, uint32_t count, uint32_t flags, Creature*) const
{
	...
	if((item->getSlotPosition() & SLOTP_HEAD) || (item->getSlotPosition() & SLOTP_NECKLACE) ||
		...
		(item->getSlotPosition() & SLOTP_RING2) || (item->getSlotPosition() & SLOTP_BELT))
	...
	switch(index)
	{
		...
		case SLOT_AMMO:
			...
			break;
		case SLOT_RING2:
			if(item->getSlotPosition() & SLOTP_RING2)
				ret = RET_NOERROR;
			break;
		case SLOT_BELT:
			if(item->getSlotPosition() & SLOTP_BELT || g_config.getBool(ConfigManager::TIBIA_SLOTS))
				ret = RET_NOERROR;
			break;			
		case SLOT_WHEREEVER:
		case -1:
...
void Player::__addThing(Creature*, int32_t index, Thing* thing)
{
	if(index < 0 || index > 13)
	{
		...
		std::clog << "Failure: [Player::__addThing], " << "player: " << getName() << ", index: " << index << ", index < 0 || index > 13" << std::endl;
#endif
		return /*RET_NOTPOSSIBLE*/;
	}
...
void Player::__replaceThing(uint32_t index, Thing* thing)
{
	...
	{
#ifdef __DEBUG_MOVESYS__
		std::clog << "Failure: [Player::__replaceThing], " << "player: " << getName() << ", index: " << index << ", index < 0 || index > 13" << std::endl;
#endif
		return /*RET_NOTPOSSIBLE*/;
	}
...
void Player::__internalAddThing(uint32_t index, Thing* thing)
{
	...
	{
#ifdef __DEBUG_MOVESYS__
		std::clog << "Failure: [Player::__internalAddThing] index == 0 || index > 13" << std::endl;
#endif
		return;
	}
...
void Player::doAttacking(uint32_t)
{
	...
	std::string value;
	this->getStorage("attackspeed", value);
	int aspeed = atoi(value.c_str());
	if(aspeed > 500)
		attackSpeed = aspeed;
...
void Player::changeShield(int32_t shieldChange)
{
	Creature::changeShield(shieldChange);
	sendStats();
}

void Player::changeBarrier(int32_t barrierChange)
{
...
bool Player::addOfflineTrainingTries(skills_t skill, int32_t tries)
{
	...
	else
	{
		...
		ss << "You advanced from " << getSkillName(skill) << " " << skills[skill][SKILL_LEVEL]-1 << " to " << getSkillName(skill) << " " << skills[skill][SKILL_LEVEL] << ".";
		sendTextMessage(MSG_EVENT_ADVANCE, ss.str().c_str());
		ss.str("");
	}
