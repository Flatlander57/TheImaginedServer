...
CreatureEventType_t CreatureEvents::getType(const std::string& type)
{
	...
	if(type == "login")
		...
		_type = CREATURE_EVENT_MODECHANGE;
...
std::string CreatureEvent::getScriptEventName() const
{
	switch(m_type)
	{
		...
		case CREATURE_EVENT_DISMOUNT:
			return "onDismount";
		case CREATURE_EVENT_MODECHANGE:
			return "onModeChange";
...
uint32_t CreatureEvent::executeModeChange(Player* player)
{
	//onModeChange(cid)
	if(m_interface->reserveEnv())
	{
	...
	{
		std::clog << "[Error - CreatureEvent::executeModeChange] Call stack overflow." << std::endl;
		return 0;
	}
}
