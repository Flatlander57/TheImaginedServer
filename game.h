...
class Game
{
	public:
		...
		bool combatChangeBarrier(Creature* attacker, Creature* target, int32_t barrierChange,
			CombatType_t combatType = COMBAT_MANADRAIN, bool inherited = false);
		bool doShieldMessage(Creature* attacker, Creature* target, int32_t shieldLoss);

		//animation help functions
		void addCreatureHealth(const Creature* target);
		void addCreatureHealth(const SpectatorVec& list, const Creature* target);
		void addCreatureShield(const Creature* target);
		void addCreatureBarrier(const SpectatorVec& list, const Creature* target);
		void addCreatureBarrier(const Creature* target);
		void addCreatureShield(const SpectatorVec& list, const Creature* target);
	...
	protected:
		...
		static const int32_t light_hour = 0;
