		...
		if(m_eventType == MOVE_EVENT_EQUIP || m_eventType == MOVE_EVENT_DE_EQUIP)
		{
			if(readXMLString(p, "slot", strValue))
			{
				...
				if(tmpStrValue == "head")
					...
					slot = SLOTP_BELT;
				else if(tmpStrValue == "finger")
					slot = SLOTP_RING | SLOTP_RING2 ;
				else if(tmpStrValue == "pickupable")
					slot = SLOTP_RIGHT | SLOTP_LEFT | SLOTP_AMMO | SLOTP_BELT;
...
bool MoveEvent::EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot, bool isCheck)
{
	...
	if(it.abilities->regeneration)
	{
		...
		if(it.abilities->shieldGain)
			condition->setParam(CONDITIONPARAM_SHIELDGAIN, it.abilities->shieldGain);

		if(it.abilities->shieldTicks)
			condition->setParam(CONDITIONPARAM_SHIELDTICKS, it.abilities->shieldTicks);
