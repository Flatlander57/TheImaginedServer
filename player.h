...
enum playerinfo_t
{
	...
	PLAYERINFO_MAXMANA,
	PLAYERINFO_SHIELD,
	PLAYERINFO_MAXSHIELD,
	PLAYERINFO_BARRIER,
	PLAYERINFO_MAXBARRIER,
...
class Player : public Creature, public Cylinder
{
	public:
		...
		virtual int32_t getMaxShield() const {return getPlayerInfo(PLAYERINFO_MAXSHIELD);}
		...
		void sendCreaturePShield(const Creature* creature)
			{if(client) client->sendCreaturePShield(creature);}
		...
		void sendCreatureBarrier(const Creature* creature) const
			{if(client) client->sendCreatureBarrier(creature);}
	...
	protected:
		...
		void updateBaseSpeed()
		{
			...
			{
				if(g_config.getBool(ConfigManager::LEVEL_GAIN_SPEED))
