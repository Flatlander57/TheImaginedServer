...
bool Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
	...
	{
		_params.element.damage *= g_config.getDouble(ConfigManager::PLAYER_DAMAGE_MULT);
		if(change < 0)
			change *= g_config.getDouble(ConfigManager::PLAYER_DAMAGE_MULT);
	}
...
bool Combat::CombatShieldFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
	int32_t change = 0;
	if(Combat2Var* var = (Combat2Var*)data)
	{
	...
	if(!g_game.doShieldMessage(caster, target, change))
		return false;
...
void Combat::doCombat(Creature* caster, Creature* target) const
{
	...
	if(params.combatType != COMBAT_NONE)
	{
		...
		else if (params.combatType == COMBAT_SHIELD)
			doCombatShield(caster, target, minChange, maxChange, _params, false);
		else
...
void Combat::doCombat(Creature* caster, const Position& pos) const
{
	...
	if(params.combatType != COMBAT_NONE)
	{
		...
		if(params.combatType == COMBAT_MANADRAIN)
			...
			doCombatShield(caster, pos, area, minChange, maxChange, _params);	
		else
...
void Combat::doCombatShield(Creature* caster, Creature* target, int32_t minChange, int32_t maxChange, const CombatParams& params, bool check/* = true*/)
{
	if(check && params.isAggressive && (caster == target || Combat::canDoCombat(caster, target, true) != RET_NOERROR))
		return;
	...
	CombatShieldFunc(caster, target, params, (void*)&var);
	if(params.targetCallback)
		params.targetCallback->onTargetCombat(caster, target);
...
void Combat::doCombatShield(Creature* caster, const Position& pos, const CombatArea* area,
	int32_t minChange, int32_t maxChange, const CombatParams& params)
{
	Combat2Var var;
	var.minChange = minChange;
	var.maxChange = maxChange;
	CombatFunc(caster, pos, area, params, CombatShieldFunc, (void*)&var);
}
