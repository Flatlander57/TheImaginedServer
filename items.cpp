...
void Items::parseItemNode(xmlNodePtr itemNode, uint32_t id)
{
	...
	for(xmlNodePtr itemAttributesNode = itemNode->children; itemAttributesNode; itemAttributesNode = itemAttributesNode->next)
	{
		...
		else if(tmpStrValue == "slottype")
		{
			if(readXMLString(itemAttributesNode, "value", strValue))
			{
				...
				else if(tmpStrValue == "ring2")
				{
					...
					it.wieldPosition = SLOT_RING2;
				}
				else if(tmpStrValue == "belt")
				{
					it.slotPosition |= SLOTP_BELT;
					it.wieldPosition = SLOT_BELT;
				}
		...
		else if(tmpStrValue == "shieldgain")
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
			{
				it.getAbilities()->regeneration = true;
				it.getAbilities()->shieldGain = intValue;
			}
		}
		else if(tmpStrValue == "shieldticks")
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
			{
				it.getAbilities()->regeneration = true;
				it.getAbilities()->shieldTicks = intValue;
			}
		}
		...
		else if(tmpStrValue == "maxshieldpoints")
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
				it.getAbilities()->stats[STAT_MAXSHIELD] = intValue;
		}
		else if(tmpStrValue == "maxshieldpercent")
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
				it.getAbilities()->statsPercent[STAT_MAXSHIELD] = intValue;
		}
