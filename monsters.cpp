...
void MonsterType::reset()
{
	...
	runAwayHealth = healthMin = manaMin = shieldMin = barrierMin = manaCost = lightLevel = lightColor = yellSpeedTicks = yellChance = changeTargetSpeed = changeTargetChance = 0;
...
bool Monsters::deserializeSpell(xmlNodePtr node, spellBlock_t& sb, const std::string& description)
{
	...
	if(readXMLInteger(node, "mana", intValue))
		sb.mana = intValue;
		
	if(readXMLInteger(node, "health", intValue))
		sb.health = intValue;
	...
	else
	{
		...
		else if(tmpName == "skills" || tmpName == "attributes")
		{
			...
			if(readXMLInteger(node, "melee", intValue))
			...
			else if(readXMLInteger(node, "maxshield", intValue))
				param = CONDITIONPARAM_STAT_MAXSHIELDPERCENT;
				...
				param = CONDITIONPARAM_STAT_MAXMANAPERCENT;
			else if(readXMLInteger(node, "maxshieldPercent", intValue))
				param = CONDITIONPARAM_STAT_MAXSHIELDPERCENT;
...
bool Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading/* = false*/)
{
	...
	for(xmlNodePtr p = root->children; p; p = p->next)
	{
		...
		if(!xmlStrcmp(p->name, (const xmlChar*)"health"))
		{
			...
			if(readXMLInteger(p, "regen", intValue))
				mType->healthRegen = intValue;
		}
		else if(!xmlStrcmp(p->name, (const xmlChar*)"mana"))
		{
			if(readXMLInteger(p, "min", intValue))
				mType->manaMin = intValue;

			if(readXMLInteger(p, "max", intValue))
				mType->manaMax = intValue;

			if(!readXMLInteger(p, "now", intValue))
				mType->mana = mType->manaMax;
			else
				mType->mana = intValue;
				
			if(readXMLInteger(p, "regen", intValue))
				mType->manaRegen = intValue;
		}
		else if(!xmlStrcmp(p->name, (const xmlChar*)"shield"))
		{
			if(readXMLInteger(p, "min", intValue))
				mType->shieldMin = intValue;

			if(!readXMLInteger(p, "max", intValue))
			{
				SHOW_XML_ERROR("Missing Shield.max");
				monsterLoad = false;
				break;
			}

			mType->shieldMax = intValue;
			if(!readXMLInteger(p, "now", intValue))
				mType->shield = mType->shieldMax;
			else
				mType->shield = intValue;
			
			if(readXMLInteger(p, "regen", intValue))
				mType->shieldRegen = intValue;
		}
		else if(!xmlStrcmp(p->name, (const xmlChar*)"flags"))
		{
			for(xmlNodePtr tmpNode = p->children; tmpNode; tmpNode = tmpNode->next)
			{
				if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"flag"))
				{
					...
					if(readXMLInteger(tmpNode, "alwaysactive", intValue))
						mType->alwaysactive = intValue;
					else 
						mType->alwaysactive = 0;
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"aggro"))
		{
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"groups"))
		{
			if(readXMLString(p, "name", strValue))
				mType->group = strValue;
			else
				mType->group = "default";
			
			if(readXMLInteger(p, "default", intValue))
				mType->groupType = intValue;
			else
				mType->groupType = 0;
				
			if(readXMLInteger(p, "players", intValue))
				mType->groupPlayer = intValue;
			else
				mType->groupPlayer = 2;
				
			for(xmlNodePtr tmpNode = p->children; tmpNode; tmpNode = tmpNode->next)
			{
				if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"group"))
				{
					uint32_t status = 0;
					if(readXMLInteger(tmpNode, "status", intValue))
						status = intValue;

					if(readXMLString(tmpNode, "name", strValue))
					{		
						groupBlock_t gb;
						gb.groupname = strValue;
						gb.status = status;

						mType->groupList.push_back(gb);
					}
				}
			}
		}
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"names"))
		{
			for(xmlNodePtr tmpNode = p->children; tmpNode; tmpNode = tmpNode->next)
			{
				if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"monstername"))
				{
					nameBlock_t nb;
					nb.name = "";
					nb.nameDescription = "";
					if(readXMLString(tmpNode, "name", strValue))
						...
						SHOW_XML_WARNING("Missing monstername.name");

					if(readXMLString(tmpNode, "description", strValue))
						nb.nameDescription = strValue;
						
					mType->nameVector.push_back(nb);
				}
			}
		}
