...
bool Creature::canSee(const Position& pos) const
{
	...
	if (number < Map::maxViewportX)
		number = Map::maxViewportX;
		
	return canSee(getPosition(), pos, number, number);
}
...
void Creature::changeShield(int32_t shieldChange)
{
	if(shieldChange > 0)
		shield += std::min(shieldChange, getMaxShield() - shield);
	else
		shield = std::max((int32_t)0, shield + shieldChange);
		
	g_game.addCreatureShield(this);
}

void Creature::changeBarrier(int32_t barrierChange)
{
	if(barrierChange > 0)
		barrier += std::min(barrierChange, getMaxBarrier() - barrier);
	else
		barrier = std::max((int32_t)0, barrier + barrierChange);
		
	g_game.addCreatureBarrier(this);
}
	...
	bool checkDefense/* = false*/, bool checkArmor/* = false*/, bool/* reflect = true*/, bool/* field = false*/, bool/* element = false*/)
{
	...
	else if(checkDefense || checkArmor)
	{
		...
		{
			int32_t maxDefense = getDefense(), minDefense = maxDefense;
			...
			if(damage <= 0)
			{
				...
				if(g_config.getBool(ConfigManager::USE_SHIELD_BAR))
					g_game.doShieldMessage(attacker, this, reduct);
				damage = 0;
				blockType = BLOCK_DEFENSE;
				checkArmor = false;
			}
			else if(g_config.getBool(ConfigManager::USE_SHIELD_BAR))
			{
				g_game.doShieldMessage(attacker, this, reduct);
	
				CreatureEventList statsChangeEvents = this->getCreatureEvents(CREATURE_EVENT_STATSCHANGE);
				for(CreatureEventList::iterator it = statsChangeEvents.begin(); it != statsChangeEvents.end(); ++it)
				{
					if(!(*it)->executeStatsChange(this, attacker, STATSCHANGE_SHIELDLOSS, COMBAT_SHIELD, reduct))
						continue;
				}
			}
		}
		...
		if(checkArmor)
		{
			...
			if(armorValue > 1)
			{
				minArmorReduction = (int32_t)std::ceil(armorValue * g_config.getDouble(ConfigManager::ARMOR_MIN));
				maxArmorReduction = (int32_t)std::ceil((armorValue * g_config.getDouble(ConfigManager::ARMOR_MAX)) - 1);
			}
