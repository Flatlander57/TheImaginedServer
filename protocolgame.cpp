...
void ProtocolGame::GetTileDescription(const Tile* tile, NetworkMessage_ptr msg)
{
	...
	if(creatures)
	{
		for(CreatureVector::const_reverse_iterator cit = creatures->rbegin(); (cit != creatures->rend() && count < 10); ++cit)
		{
			...
			if((g_config.getBool(ConfigManager::SEE_THROUGH_WALLS)) && (!g_game.isSightClear(player->getPosition(), (*cit)->getPosition(), false)))
				continue;
...
bool ProtocolGame::canSee(uint16_t x, uint16_t y, uint16_t z) const
{
	...
	return ((x >= myPos.x - 11 + offsetz) && (x <= myPos.x + 12 + offsetz) &&
		(y >= myPos.y - 9 + offsetz) && (y <= myPos.y + 10 + offsetz));
}
...
void ProtocolGame::parseFightModes(NetworkMessage& msg)
{
	...
	uint8_t rawChaseMode = msg.get<char>(); // 0 - stand while fighting, 1 - chase opponent
...
void ProtocolGame::sendCreaturePShield(const Creature* creature)
{
	if(!canSee(creature))
		return;
...
void ProtocolGame::sendCreatureShield(const Creature* creature)
{
	...
	TRACK_MESSAGE(msg);
	AddCreatureShield(msg, creature);
}

void ProtocolGame::sendCreatureBarrier(const Creature* creature)
{
	if(!canSee(creature))
		return;
	...
	TRACK_MESSAGE(msg);
	AddCreatureBarrier(msg, creature);
}
...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos, uint32_t stackpos)
{
	...
	if((g_config.getBool(ConfigManager::SEE_THROUGH_WALLS)) && (!g_game.isSightClear(player->getPosition(), pos, false)))
		return;
	...
	uint32_t newStackpos, const Tile*, const Position& oldPos, uint32_t oldStackpos, bool teleport)
{
	if(creature == player)
	{
		...
		else
		{
			...
			{
				msg->put<char>(0x65);
				GetMapDescription(oldPos.x - 11, newPos.y - 9, newPos.z, 24, 1, msg);
			}
			else if(oldPos.y < newPos.y) // south, for old x
			{
				msg->put<char>(0x67);
				GetMapDescription(oldPos.x - 11, newPos.y + 10, newPos.z, 24, 1, msg);
			}

			if(oldPos.x < newPos.x) // east, [with new y]
			{
				msg->put<char>(0x66);
				GetMapDescription(newPos.x + 12, newPos.y - 9, newPos.z, 1, 20, msg);
			}
			else if(oldPos.x > newPos.x) // west, [with new y]
			{
				msg->put<char>(0x68);
				GetMapDescription(newPos.x - 11, newPos.y - 9, newPos.z, 1, 20, msg);
			}
			
			if((oldPos.y == newPos.y) && (g_config.getBool(ConfigManager::SEE_THROUGH_WALLS)))
			{
				//get nearby creatures newPos
				SpectatorVec list;
				SpectatorVec::iterator it;
				g_game.getSpectators(list, newPos, false, false, 12, 12, 10, 10);

				for(it = list.begin(); it != list.end(); ++it)
				{
					if((*it) != creature)
					{
						if((newPos.y - (*it)->getPosition().y) == 10)
							continue;
							
						if((newPos.x - (*it)->getPosition().x) == 12)
							continue;
							
						//If creature disappear
						if(g_game.isSightClear(oldPos, (*it)->getPosition(), false) && !g_game.isSightClear(newPos, (*it)->getPosition(), false))
						{
							...
							if(!msg)
								...
								TRACK_MESSAGE(msg);
								RemoveTileItem(msg, (*it)->getPosition(), ((*it)->getTile()->getClientIndexOfThing(player, (*it))));
						}
						else if(!g_game.isSightClear(oldPos, (*it)->getPosition(), false) && g_game.isSightClear(newPos, (*it)->getPosition(), false))
						{
							NetworkMessage_ptr msg = getOutputBuffer();
							if(!msg)
								return;
								
								int32_t index = (*it)->getTile()->__getIndexOfThing(*it);
								TRACK_MESSAGE(msg);
								AddTileCreature(msg, (*it)->getPosition(), index, (*it));
						}
					}
				}
			}
			
			else if((oldPos.x == newPos.x) && (g_config.getBool(ConfigManager::SEE_THROUGH_WALLS)))
			{
				//get nearby creatures newPos
				SpectatorVec list;
				SpectatorVec::iterator it;
				g_game.getSpectators(list, newPos, false, false, 12, 12, 9, 9);
			...
			else if(g_config.getBool(ConfigManager::SEE_THROUGH_WALLS))
			{
				//get nearby creatures newPos
				SpectatorVec list;
				SpectatorVec::iterator it;
				g_game.getSpectators(list, newPos, false, false, 11, 11, 9, 9);
	...
	else if(canSee(oldPos) && canSee(newPos)) //Creature is walking, and is on Player Screen
	{
		if(!player->canSeeCreature(creature))
			return;
		
		if ((g_game.isSightClear(player->getPosition(), oldPos, false) && g_game.isSightClear(player->getPosition(), newPos, false)) || (!g_config.getBool(ConfigManager::SEE_THROUGH_WALLS))) //Player can see the fromPos and toPos
		{
			NetworkMessage_ptr msg = getOutputBuffer();
			if(!msg)
				return;
		...
		else if (g_game.isSightClear(player->getPosition(), newPos, false)) //Player can only see the toPos (Creature needs to appear)
		{
			NetworkMessage_ptr msg = getOutputBuffer();
			if(!msg)
				return;
		...
		else if (g_game.isSightClear(player->getPosition(), oldPos, false)) //Player can only see the fromPos (Creature needs to disapear)
		{
			if(!player->canSeeCreature(creature))
				return;
	...
	else if(canSee(oldPos))
	{
		...
		if((g_config.getBool(ConfigManager::SEE_THROUGH_WALLS)) && (!g_game.isSightClear(player->getPosition(), oldPos, false)))
			return;
...
void ProtocolGame::AddMapDescription(NetworkMessage_ptr msg, const Position& pos)
{
	...
	GetMapDescription(pos.x - 11, pos.y - 9, pos.z, 24, 20, msg);
}
...
void ProtocolGame::AddCreatureShield(NetworkMessage_ptr msg,const Creature* creature)
{
	msg->put<char>(0x8B);
	msg->put<uint32_t>(creature->getID());
	if(!creature->getHideHealth())
		msg->put<char>((uint8_t)std::ceil(creature->getShield() * 100. / std::max(creature->getMaxShield(), (int32_t)1)));
	else
		msg->put<char>(0x00);
}

void ProtocolGame::AddCreatureBarrier(NetworkMessage_ptr msg,const Creature* creature)
{
	msg->put<char>(0x8A);
	msg->put<uint32_t>(creature->getID());
	if(!creature->getHideHealth())
		msg->put<char>((uint8_t)std::ceil(creature->getBarrier() * 100. / std::max(creature->getMaxBarrier(), (int32_t)1)));
	else
		msg->put<char>(0x00);
}
