...
bool Vocations::parseVocationNode(xmlNodePtr p)
{
	...
	if(readXMLInteger(p, "gainshield", intValue))
		voc->setGain(GAIN_SHIELD, intValue);
	...
	if(readXMLInteger(p, "gainshieldticks", intValue))
		voc->setGainTicks(GAIN_SHIELD, intValue);

	if(readXMLInteger(p, "gainshieldamount", intValue))
		voc->setGainAmount(GAIN_SHIELD, intValue);
