...
int32_t Weapons::getMaxWeaponDamage(int32_t level, int32_t attackSkill, int32_t attackValue, float attackFactor)
{
	if(!g_config.getBool(ConfigManager::USE_SKILLS))
		return (int32_t)std::ceil((2 * (attackValue * (10 + 5.8) / 25 + (level - 1) / 10.)) / attackFactor);
	else
...
bool Weapon::useFist(Player* player, Creature* target)
{
	...
	double attackFactor = player->getAttackFactor();
	...
	if(!player->hasFlag(PlayerFlag_NotGainSkill) && player->getAddAttackSkill() && g_config.getBool(ConfigManager::USE_SKILLS))
...
{
	if(!player->hasFlag(PlayerFlag_NotGainSkill) &&  g_config.getBool(ConfigManager::USE_SKILLS))
	{
		skills_t skillType;
