...
bool ConfigManager::load()
{
	...
	m_confNumber[LONG_DAY] = getGlobalNumber("longday", 3600); //60000?
	m_confNumber[DAY_PERCENT] = getGlobalNumber("daypercent", 50);
	m_confNumber[NIGHT_LIGHT] = getGlobalNumber("nightlight", 40);
	m_confNumber[DAY_LIGHT] = getGlobalNumber("daylight", 250);
	m_confNumber[LIGHT_COLOR] = getGlobalNumber("lightcolor", 215);
	m_confBool[SEE_THROUGH_WALLS] = getGlobalBool("seethroughwalls", false);	
	m_confBool[SHOW_PLAYER_VOCATION] = getGlobalBool("showPlayerVocation", true);
	m_confBool[SHOW_PLAYER_MARRIAGE] = getGlobalBool("showPlayerMarriage", true);
	m_confBool[SHOW_PLAYER_GUILD] = getGlobalBool("showPlayerGuild", true);
	m_confBool[MONSTER_TARGET_DEFAULT] = getGlobalBool("monsterTargetDefault", true);
	m_confNumber[ACCOUNT_SIZE] = getGlobalNumber("accountSize", 15);
	m_confDouble[DEFENSE_MIN] = getGlobalDouble("defenseMin", 0.5);
	m_confDouble[DEFENSE_MAX] = getGlobalDouble("defenseMax", 1);
	m_confDouble[ARMOR_MIN] = getGlobalDouble("armorMin", 0.5);
	m_confDouble[ARMOR_MAX] = getGlobalDouble("armorMax", 1);
	m_confDouble[OFFENSE_DAMAGE] = getGlobalDouble("offenseDamage", 1.0);
	m_confDouble[BALANCED_DAMAGE] = getGlobalDouble("balancedDamage", 1.2);
	m_confDouble[DEFENSE_DAMAGE] = getGlobalDouble("defenseDamage", 2.0);
	m_confDouble[OFFENSE_DEFENSE] = getGlobalDouble("offenseDefense", 1.0);
	m_confDouble[BALANCED_DEFENSE] = getGlobalDouble("balancedDefense", 1.2);
	m_confDouble[DEFENSE_DEFENSE] = getGlobalDouble("defenseDefense", 2.0);
	m_confDouble[PLAYER_DAMAGE_MULT] = getGlobalDouble("playerDamageMult", 0.5);
	m_confBool[USE_SKILLS] = getGlobalBool("useSkills", true);
	m_confBool[LEVEL_GAIN_SPEED] = getGlobalBool("levelGainSpeed", true);
	m_confBool[CAP_SPEED_LOSS] = getGlobalBool("capSpeedLoss", false);
	m_confBool[USE_SHIELD_BAR] = getGlobalBool("useShieldBar", false);
	m_confBool[USE_BARRIER_BAR] = getGlobalBool("useBarrierBar", false);
