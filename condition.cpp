...
bool ConditionAttributes::setParam(ConditionParam_t param, int32_t value)
{
	...
	switch(param)
	{
		...
		case CONDITIONPARAM_STAT_MAXSHIELD:
			stats[STAT_MAXSHIELD] = value;
			return true;
		...
		case CONDITIONPARAM_STAT_MAXSHIELDPERCENT:
			statsPercent[STAT_MAXSHIELD] = std::max((int32_t)0, value);
			return true;
...
ConditionGeneric(_id, _type, _ticks, _buff, _subId)
{
	internalHealthTicks = internalManaTicks = internalShieldTicks = healthGain = manaGain = shieldGain = 0;
	healthTicks = manaTicks = shieldTicks = 1000;
}
...
void ConditionRegeneration::addCondition(Creature*, const Condition* addCondition)
{
	...
	shieldTicks = conditionRegen.shieldTicks;
	
	healthGain = conditionRegen.healthGain;
	manaGain = conditionRegen.manaGain;
	shieldGain = conditionRegen.shieldGain;
}
...
bool ConditionRegeneration::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	switch(attr)
	{
		...
		case CONDITIONATTR_SHIELDTICKS:
		{
			uint32_t value = 0;
			if(!propStream.getType(value))
				return false;
...
bool ConditionRegeneration::serialize(PropWriteStream& propWriteStream)
{
	...
	propWriteStream.addByte(CONDITIONATTR_SHIELDTICKS);
	propWriteStream.addType(shieldTicks);

	propWriteStream.addByte(CONDITIONATTR_SHIELDGAIN);
	propWriteStream.addType(shieldGain);	
...
bool ConditionRegeneration::executeCondition(Creature* creature, int32_t interval)
{
	...
	if(creature->getZone() != ZONE_PROTECTION)
	{
		...
		if(internalShieldTicks >= shieldTicks)
		{
			internalShieldTicks = 0;
			if(shieldGain && creature->getShield() < creature->getMaxShield())
				creature->changeShield(shieldGain);
		}
	}
...
bool ConditionRegeneration::setParam(ConditionParam_t param, int32_t value)
{
	...
	switch(param)
	{
		...
		case CONDITIONPARAM_SHIELDGAIN:
			shieldGain = value;
			return true;

		case CONDITIONPARAM_SHIELDTICKS:
			shieldTicks = value;
			return true;
