...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	<< "`maglevel`, `health`, `healthmax`, `shield`, `shieldmax`, `barrier`, `barriermax`, `blessings`, `pvp_blessing`, `mana`, `manamax`, `manaspent`, `soul`, "
	...
	player->shield = result->getDataInt("shield");
	player->shieldMax = result->getDataInt("shieldmax");
	player->barrier = result->getDataInt("barrier");
	player->barrierMax = result->getDataInt("barriermax");
	...
	if((result = db->storeQuery(query.str())))
	{
		do
		{
			...
			if(storeNameByGuid(vid))
				player->addVIP(vid, "", description, icon, notify, VIPSTATUS_ONLINE, true);
		}
		while(result->next());
		result->free();
	}
	...
	player->updateInventoryWeight(false);
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	query << "`shield` = " << player->shield << ", ";
	query << "`shieldmax` = " << player->shieldMax << ", ";
	query << "`barrier` = " << player->barrier << ", ";
	query << "`barriermax` = " << player->barrierMax << ", ";
...
bool IOLoginData::createCharacter(uint32_t accountId, std::string characterName, int32_t vocationId, uint16_t sex)
{
	...
	uint16_t healthMax = 150, manaMax = 0,  shieldMax = 0, barrierMax = 1000, capMax = 400, lookType = 136;
	if(sex % 2)
		lookType = 128;
	...
	if(tmpLevel > 0)
	{
		...
		shieldMax += rookVoc->getGain(GAIN_SHIELD) * tmpLevel;
		...
		if(level > 8)
		{
			...
			shieldMax += vocation->getGain(GAIN_SHIELD) * tmpLevel;
			capMax += vocation->getGainCap() * tmpLevel;
		}
	}

	query.str("");
	query << "INSERT INTO `players` (`id`, `name`, `world_id`, `group_id`, `account_id`, `level`, `vocation`, `health`, `healthmax`, `shield`, `shieldmax`, `experience`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `maglevel`, `mana`, `manamax`, `manaspent`, `soul`, `town_id`, `posx`, `posy`, `posz`, `conditions`, `cap`, `sex`, `lastlogin`, `lastip`, `skull`, `skulltime`, `save`, `rank_id`, `guildnick`, `lastlogout`, `blessings`, `online`) VALUES (NULL, " << db->escapeString(characterName) << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", 1, " << accountId << ", " << level << ", " << vocationId << ", " << healthMax << ", " << healthMax << ", " << shieldMax << ", " << shieldMax << ", " << exp << ", 68, 76, 78, 39, " << lookType << ", 0, " << g_config.getNumber(ConfigManager::START_MAGICLEVEL) << ", " << manaMax << ", " << manaMax << ", 0, 100, " << g_config.getNumber(ConfigManager::SPAWNTOWN_ID) << ", " << g_config.getNumber(ConfigManager::SPAWNPOS_X) << ", " << g_config.getNumber(ConfigManager::SPAWNPOS_Y) << ", " << g_config.getNumber(ConfigManager::SPAWNPOS_Z) << ", 0, " << capMax << ", " << sex << ", 0, 0, 0, 0, 1, 0, '', 0, 0, 0)";
	return db->query(query.str());
}
