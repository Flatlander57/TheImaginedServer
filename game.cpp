...
Game::Game()
{
	...
	lightHourDelta = 1440 * 10;
	lightHour = light_hour;
	lightLevel = 1;
	...
	if(player->hasCustomFlag(PlayerCustomFlag_CanSeeCreatureDetails))
	{
		if(const Creature* creature = thing->getCreature())
		{
			if(!player->hasFlag(PlayerFlag_HideHealth))
			{
				...
				ss << std::endl << "Shield: [" << creature->getShield() << " / " << creature->getMaxShield() << "]";
...
bool Game::playerSetFightModes(uint32_t playerId, fightMode_t fightMode, chaseMode_t chaseMode, secureMode_t secureMode)
{
	...
	CreatureEventList modesChangeEvents = player->getCreatureEvents(CREATURE_EVENT_MODECHANGE);
	for(CreatureEventList::iterator it = modesChangeEvents.begin(); it != modesChangeEvents.end(); ++it)
	{
		if(!(*it)->executeModeChange(player))
			continue;
	}
	...
	bool ghostMode, SpectatorVec* spectators/* = NULL*/, Position* pos/* = NULL*/, uint32_t statementId/* = 0*/)
{
	...
	if(!spectators || !spectators->size())
	{
		...
		if(type != MSG_SPEAK_YELL && type != MSG_SPEAK_MONSTER_YELL)
			...
			getSpectators(list, destPos, false, true, 22, 22, 18, 18);
	}
	else
		list = (*spectators);
	...
	else
	{
		...
		if(damage > 0)
		{
			if(target->getBarrier() > 0 && g_config.getBool(ConfigManager::USE_BARRIER_BAR))
			{
				...
				if(barrierDamage && combatChangeBarrier(attacker, target, -barrierDamage, params.combatType, true))
...
bool Game::combatChangeBarrier(Creature* attacker, Creature* target, int32_t barrierChange,
	CombatType_t combatType/* = COMBAT_MANADRAIN*/, bool inherited/* = false*/)
{
	const Position& targetPos = target->getPosition();
	if(barrierChange > 0)
	{
		...
		for(CreatureEventList::iterator it = statsChangeEvents.begin(); it != statsChangeEvents.end(); ++it)
		{
			if(!(*it)->executeStatsChange(target, attacker, STATSCHANGE_BARRIERGAIN, COMBAT_HEALING, barrierChange))
				deny = true;
		}
		...
		if(oldBarrier != target->getBarrier() && !target->isGhost() || !target->getMonster())
		{
			...
			barrierChange = (target->getBarrier() - oldBarrier);
			std::string plural = (barrierChange != 1 ? "s." : ".");

			std::stringstream ss;
			MessageDetails* details = new MessageDetails(barrierChange, COLOR_ORANGE);
			if(!textList.empty())
			{
				if(!attacker)
					ss << ucfirst(target->getNameDescription()) << " is protected with a barrier of " << barrierChange << " strength" << plural;
				else if(attacker != target)
					ss << ucfirst(attacker->getNameDescription()) << " protects " << target->getNameDescription() << " with a barrier of " << barrierChange << " strength" << plural;
				else
				{
					ss << ucfirst(attacker->getNameDescription()) << " protects ";
					if(Player* attackerPlayer = attacker->getPlayer())
						ss << (attackerPlayer->getSex(false) == PLAYERSEX_FEMALE ? "herself" : "himself") << " with a barrier of " << barrierChange << " strength" << plural;
					else
						ss << "itself with a barrier of " << barrierChange << " strength" << plural;
				}
			...
			{
				if(attacker != target)
					ss << "You protect " << target->getNameDescription() << " with a barrier of " << barrierChange << " strength" << plural;
				else
					ss << "You protect yourself with a barrier of " << barrierChange << " strength" << plural;
			...
			{
				if(attacker)
					ss << ucfirst(attacker->getNameDescription()) << " protects you with a barrier of " << barrierChange << " strength" << plural;
				else
					ss << "You are protected with a barrier of " << barrierChange << " strength" << plural;
	...
	{
		int32_t barrierLoss = std::min(target->getBarrier(), -barrierChange);
		if(barrierLoss > 0)
		{
			bool deny = false;
			CreatureEventList statsChangeEvents = target->getCreatureEvents(CREATURE_EVENT_STATSCHANGE);
			for(CreatureEventList::iterator it = statsChangeEvents.begin(); it != statsChangeEvents.end(); ++it)
			{
				if(!(*it)->executeStatsChange(target, attacker, STATSCHANGE_BARRIERLOSS, combatType, barrierLoss))
					deny = true;
			}
			...
			target->changeBarrier(-barrierLoss);
			const SpectatorVec& list = getSpectators(targetPos);
			...
			std::stringstream ss;
			MessageDetails* details = new MessageDetails(barrierLoss, COLOR_DARKORANGE);
			if(!textList.empty())
			{
				if(!attacker)
					ss << ucfirst(target->getNameDescription()) << " loses " << barrierLoss << " barrier.";
				else if(attacker != target)
					ss << ucfirst(target->getNameDescription()) << " loses " << barrierLoss << " barrier due to an attack by " << attacker->getNameDescription();
				else
					ss << ucfirst(target->getNameDescription()) << " loses " << barrierLoss << " barrier due to a self attack.";
			...
			{
				if(attacker != target)
					ss << ucfirst(target->getNameDescription()) << " loses " << barrierLoss << " barrier due to your attack.";
				else
					ss << "You lose " << barrierLoss << " barrier due to your attack.";
			...
			{
				if(attacker)
					ss << "You lose " << barrierLoss << " barrier due to an attack by " << attacker->getNameDescription();
				else
					ss << "You lose " << barrierLoss << " barrier.";
...
bool Game::doShieldMessage(Creature* attacker, Creature* target, int32_t shieldLoss)
{
	const Position& targetPos = target->getPosition();
	int32_t oldShield = target->getShield();
	target->changeShield(-shieldLoss);
	if(shieldLoss > 0)
	{
		const SpectatorVec& list = getSpectators(targetPos);
		...
		std::stringstream ss;
		MessageDetails* details = new MessageDetails(shieldLoss, COLOR_GREY);
		if(!textList.empty())
		{
			if(!attacker)
				ss << ucfirst(target->getNameDescription()) << " loses " << shieldLoss << " shield.";
			else if(attacker != target)
				ss << ucfirst(target->getNameDescription()) << " loses " << shieldLoss << " shield due to an attack by " << attacker->getNameDescription();
			else
				ss << ucfirst(target->getNameDescription()) << " loses " << shieldLoss << " shield due to a self attack.";
		...
		{
			if(attacker != target)
				ss << ucfirst(target->getNameDescription()) << " loses " << shieldLoss << " shield due to your attack.";
			else
				ss << "You lose " << shieldLoss << " shield due to your attack.";
		...
		{
			if(attacker)
				ss << "You lose " << shieldLoss << " shield due to an attack by " << attacker->getNameDescription();
			else
				ss << "You lose " << shieldLoss << " shield.";
...
void Game::addCreatureShield(const Creature* target)
{
	const SpectatorVec& list = getSpectators(target->getPosition());
	addCreatureShield(list, target);
}

void Game::addCreatureShield(const SpectatorVec& list, const Creature* target)
{
	Player* player = NULL;
	for(SpectatorVec::const_iterator it = list.begin(); it != list.end(); ++it)
	{
		if((player = (*it)->getPlayer()))
			player->sendCreatureShield(target);
	}
}

void Game::addCreatureBarrier(const Creature* target)
{
	const SpectatorVec& list = getSpectators(target->getPosition());
	addCreatureBarrier(list, target);
}

void Game::addCreatureBarrier(const SpectatorVec& list, const Creature* target)
{
	Player* player = NULL;
	for(SpectatorVec::const_iterator it = list.begin(); it != list.end(); ++it)
	{
		if((player = (*it)->getPlayer()))
			player->sendCreatureBarrier(target);
	}
}
...
void Game::checkLight()
{
	...
	lightHour = (lightHour + lightHourDelta / g_config.getNumber(ConfigManager::LONG_DAY));
	if(lightHour > 1440)
		lightHour -= 1440;
		
	if(std::abs(lightHour) < (1440 / (100 / g_config.getNumber(ConfigManager::DAY_PERCENT))))
	...
	if(newLightLevel <= g_config.getNumber(ConfigManager::NIGHT_LIGHT))
	{
		lightLevel = g_config.getNumber(ConfigManager::NIGHT_LIGHT);
		lightState = LIGHT_STATE_NIGHT;
		lightChange = true;
	}
	else if(newLightLevel >= g_config.getNumber(ConfigManager::DAY_LIGHT))
	{
		lightLevel = g_config.getNumber(ConfigManager::DAY_LIGHT);
		lightState = LIGHT_STATE_DAY;
		lightChange = true;
	}
	else
...
void Game::getWorldLightInfo(LightInfo& lightInfo)
{
	...
	lightInfo.color = g_config.getNumber(ConfigManager::LIGHT_COLOR);
}
...
void Game::updateCreatureType(Creature* creature)
{
	...
	if(creatureType == CREATURETYPE_SUMMON_OTHERS)
	{
		for(SpectatorVec::const_iterator it = list.begin(), end = list.end(); it != end; ++it)
		{
			...
			//	player->sendCreatureType(creatureId, CREATURETYPE_SUMMON_OWN); //Crashes the Server on this line
			//else
...
void Game::updateCreaturePShield(Creature* creature)
{
	const SpectatorVec& list = getSpectators(creature->getPosition());
	...
	{
		if((tmpPlayer = (*it)->getPlayer()))
			tmpPlayer->sendCreaturePShield(creature);
	}
}
...
bool Game::reloadInfo(ReloadInfo_t reload, uint32_t playerId/* = 0*/, bool completeReload/* = false*/)
{
	...
	switch(reload)
	{
		...
		case RELOAD_GROUPS:
		{
			//TODO- this way needs a secure 'back', so players won't stay with empty pointers to new groups
