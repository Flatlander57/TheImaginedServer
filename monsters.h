...
struct changefit_t
{
	int32_t lhead, lbody, llegs, lfeet;
};

typedef std::list<summonBlock_t> SummonList;
typedef std::list<groupBlock_t> GroupList;
typedef std::list<spellBlock_t> SpellList;
typedef std::vector<nameBlock_t> NameVector;
typedef std::vector<changefit_t> ChangefitVector;
...
class MonsterType
{
	public:
		...
		int32_t defense, armor, health, healthMin, healthMax, healthRegen, mana, manaMin, manaRegen, manaMax, shield, shieldMin, shieldRegen, shieldMax, barrier, barrierMin, barrierMax, baseSpeed, lookCorpse, corpseUnique, corpseAction,
			...
			lightLevel, lightColor, changeTargetSpeed, changeTargetChance, groupType, groupPlayer, alwaysactive;
		uint32_t yellChance, yellSpeedTicks, staticAttackChance, manaCost;
		uint64_t experience;

		std::string name, nameDescription, file, baseAggro, viewRange, group;
		...
		ChangefitVector changefitVector;
